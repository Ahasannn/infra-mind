...
from MAR.InfraMind.metrics_watcher import start_metrics_watcher
from MAR.InfraMind.inframind_router import InfraMindRouter
from MAR.InfraMind.inframind_graph import InfraMindGraph
...
def _build_metrics_url_map(models: List[str]) -> Dict[str, str]:
    ...
    for model in models:
        ...
        logger.debug("[Metrics] {} -> {}", model, urls[model])
    if not urls:
...
class InfraMindEnv:
    ...
    def step(
        ...
        graph_kwargs = get_kwargs(topology_name, len(role_set))
        ...
        if self.dry_run:
            # Random quality for pipeline testing
            import random
            mock_quality = random.choice([0.0, 1.0])
            quality = torch.tensor([mock_quality], device=self.router.device)
            quality_meta = {"is_solved": mock_quality > 0.5, "feedback": "[dry-run]"}
        elif scorer:
