...
class InfraMindGraph(Graph):
    def init_nodes(self):
        for agent_name, llm_name, kwargs in zip(self.agent_names, self.llm_names, self.node_kwargs):
            if "InfraMindAgent" in AgentRegistry.registry:
                ...
                agent_instance = AgentRegistry.get("InfraMindAgent", **kwargs)
    ...
    def run_with_policy(
        ...
        # EDF priority: deadline = arrival_time + budget_total
        # vLLM: lower priority value → served first → earlier deadlines win
        edf_priority: int | None = None
        if arrival_time is not None:
            deadline = arrival_time + budget_total
            edf_priority = int(deadline)
        ...
        def run_node(node_id: str, round_idx: int) -> None:
            node = self.nodes[node_id]
            # Set EDF priority on the node so vLLM serves urgent requests first
            if edf_priority is not None and hasattr(node, "priority"):
                node.priority = edf_priority
            ...
            if dry_run:
                # Mock execution: random latency, placeholder response
                mock_latency = random.uniform(0.5, 3.0)
                time.sleep(0.001)  # tiny sleep so perf_counter moves
                node.outputs = [f"[dry-run mock response from {_safe_role_name(node)}]"]
                success = True
                tries = 1
                usage = {"prompt_tokens": random.randint(50, 200), "completion_tokens": random.randint(20, 100)}
                usage_tracker.consume(usage_key)
                usage_tracker.reset_context(context_token)
                transition["latency_seconds"] = mock_latency
                transition["response"] = node.outputs[0]
                transition["prompt_base"] = ""
                transition["token_counts"] = _record_token_usage(usage)
                transition["observed_ttft"] = random.uniform(0.05, 0.3)
                transition["observed_tpot"] = random.uniform(0.01, 0.05)
        ...
        try:
            ...
            # Set EDF priority on the decision node as well
            if edf_priority is not None and hasattr(decision_node, "priority"):
                decision_node.priority = edf_priority
            ...
            if dry_run:
                decision_node.outputs = ["[dry-run mock final response]"]
                decision_success = True
                usage = {"prompt_tokens": random.randint(100, 300), "completion_tokens": random.randint(30, 150)}
                usage_tracker.consume(usage_key)
                usage_tracker.reset_context(context_token)
                duration_sec = random.uniform(0.5, 2.0)
