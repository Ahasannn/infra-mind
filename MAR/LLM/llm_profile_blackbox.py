"""Loader for the blackbox model pool profile.

Mirrors ``llm_profile_full.py`` but reads from ``llm_profile_blackbox.json``.
Also provides helpers for resolving base URLs and API keys from environment.
"""
...
DEFAULT_MAX_MODEL_LEN: int = (
    _PROFILE_DATA.get("global_settings", {}).get("default_max_model_len", 16384)
)
_MAX_MODEL_LEN_MAP = {
...
DEFAULT_MAX_OUTPUT_TOKENS: int = (
    _PROFILE_DATA.get("global_settings", {}).get("default_max_output_tokens", 2048)
)
